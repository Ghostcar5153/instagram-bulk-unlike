...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // —— Main UI: Start / Stop / Reset Buttons ——
  if (startBtn) {
    startBtn.addEventListener('click', async () => {
      if (isRunning || !isOnCorrectPage) return;
      try {
  ...
  resetProgressBtn.addEventListener('click', async () => {
    if (!confirm('Reset progress counters? This will reset cycles and processed counts to 0.')) return;
    
    try {
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      await chrome.tabs.sendMessage(tab.id, { action: 'resetProgress' });
      
      // Update UI immediately
      if (cyclesSpan) cyclesSpan.textContent = '0';
...
>>>>>>> ac75923 (add persistent progress tracking and reset functionality)
